...
import {User,UserCreate,UserRead,UserResult,UserReturn,UserUpdate,} from "../interfaces";
...
const partialUpdate = async (  userId: string,  payload: UserUpdate): Promise<UserReturn> => {
  if(payload.password){
    payload.password= await hash(payload.password,10)
  }
